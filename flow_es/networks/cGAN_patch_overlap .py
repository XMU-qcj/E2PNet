...
class Generator(nn.Module):
    ...
    def __init__(self, in_channel=3, out_channel=1, patch=4):
    ...
    def forward(self, x):
        ...
        patch_x = F.interpolate(patch_x, x.size()[2:])

        out = self.net_forward(patch_x)
        out = un_patch_tensor(out, self.patch)
        out = F.interpolate(out, x.size()[2:])

        patch_size = x.size(-1) // self.patch
        start = patch_size // 2
        ...
        moved_x_patch = patch_tensor(moved_x, self.patch)
        moved_y_patch = patch_tensor(moved_y, self.patch)
        moved_x_patch = F.interpolate(moved_x_patch, x.size()[2:])
        moved_y_patch = F.interpolate(moved_y_patch, x.size()[2:])

        out_move_x = self.net_forward(moved_x_patch)
        out_move_y = self.net_forward(moved_y_patch)

        out_move_x = un_patch_tensor(out_move_x, self.patch)
        out_move_y = un_patch_tensor(out_move_y, self.patch)

        out_move_x = F.interpolate(out_move_x, x.size()[2:])
        out_move_y = F.interpolate(out_move_y, x.size()[2:])

        out_move_x = torch.cat([out_move_x[..., -start:, :], out_move_x[..., 0: - start, :]], - 2)
        out_move_y = torch.cat([out_move_y[..., -start:], out_move_y[..., 0:-start]], -1)

        return (out + out_move_x + out_move_y) / 3.0
