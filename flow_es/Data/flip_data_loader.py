...
import random
import torch
from data_input.event_utils import gen_discretized_event_volume
import cv2
from utility.result_visualization import flow2image, event2image
...
davis_list = ["./indoor_flying1_500_600.h5 ",  # /home/xhlinxm/MVSEC/indoor_flying/indoor_flying1_data.hdf5
              ...
              # "/home/xhlinxm/MVSEC/indoor_flying/indoor_flying1_data_pointGT.h5",
              # "/home/xhlinxm/MVSEC/indoor_flying/indoor_flying2_data_pointGT.h5",
              "D:\indoor_flying1_full.h5",
              "D:\indoor_flying2_full.h5",
              "D:\outdoor_day2_full.h5",
...
class MyData(Dataset):  # 这个类用于准备数据（取出所有数据）
    def __init__(self, file_index, train=True, start_time=-1):
    ...
    def get_single_item(self, ind):
        ...
        if self.train:
            ...
            if np.random.rand() < self.flip_x:
                "增加对事件坐标和"
                flip_x_event_volume = torch.flip(event_volume, dims=[2])
                flip_x_prev_image = np.flip(prev_image, axis=2)
                flip_x_next_image = np.flip(next_image, axis=2)

                flip_x_flow_frame = np.flip(flow_frame, axis=2)  # 对光流的翻转可能存在问题，要试试看
                flip_x_flow_frame[0] = -flip_x_flow_frame[0]  # 翻转后光流左右相反，数值取反
                # f_img = flow2image(flow_frame)
            if np.random.rand() < self.flip_y:
                flip_y_event_volume = torch.flip(event_volume, dims=[1])
                flip_y_prev_image = np.flip(prev_image, axis=1)
                flip_y_next_image = np.flip(next_image, axis=1)

                flip_y_flow_frame = np.flip(flow_frame, axis=1)
                flip_y_flow_frame[1] = -flip_y_flow_frame[1]  # 翻转后光流上下相反，数值取反
        ...
        # 计算mask后的GT光流
        # opt_flow_frame = torch.fr.om_numpy(opt_flow_frame)
