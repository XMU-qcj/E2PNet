...
davis_list = ["./indoor_flying1_500_600.h5 ",  # /home/xhlinxm/MVSEC/indoor_flying/indoor_flying1_data.hdf5
              ...
              "/media/XH-8T/EL_matching/MVSEC/outdoor_day1_full_new.h5",
              "/media/XH-8T/EL_matching/MVSEC/outdoor_day2_full_new.h5",  # /home/lxh/MVSEC/
...
class MyData(Dataset):  # 这个类用于准备数据（取出所有数据）
    ...
    def load(self, only_length=False):
        ...
        self.gt_flow_ts=self.gt_flow_ts[:]
        self.img_ts=self.img_ts[:]
    ...
    def __len__(self):
        if self.stack_mode == "SBT":  # 按照固定时间间隔
            ...
            events_num = self.events.shape[0]
            return (events_num - 2*self.SBN_num )
    ...
    def estimate_corresponding_gt_flow(self, start_index, end_index, bbox):
        ...
        gt_start_frame_index = np.searchsorted(self.gt_flow_ts, event_frame_start_time,
                                               side='left') - 1  # 找到图像帧最近的GT帧索引左侧
        gt_end_frame_index = np.searchsorted(self.gt_flow_ts, event_frame_end_time, side='right')
        gt_begin_time = self.gt_flow_ts[gt_start_frame_index]
        gt_next_time = self.gt_flow_ts[gt_start_frame_index + 1]
        gt_end_time = self.gt_flow_ts[gt_end_frame_index]
        ...
        scale_factor = (self.gt_flow_ts[gt_start_frame_index + 1] - event_frame_start_time) / gt_dt
        total_dt = self.gt_flow_ts[gt_start_frame_index + 1] - event_frame_start_time  # 累积已计算的时间长度
        ...
        while self.gt_flow_ts[gt_start_frame_index + 1] < event_frame_end_time:  # 如果第二帧GT的结尾还在图像帧结束之前，则继续传播
            x_flow = np.squeeze(self.gt_flow_frame[gt_start_frame_index, 0, :, :])
            y_flow = np.squeeze(self.gt_flow_frame[gt_start_frame_index, 1, :, :])
            ...
            self.prop_flow(x_flow, y_flow,
                           x_indices, y_indices,
                           x_mask, y_mask)
            total_dt += self.gt_flow_ts[gt_start_frame_index + 1] - self.gt_flow_ts[gt_start_frame_index]
            gt_start_frame_index += 1

        final_dt = event_frame_end_time - self.gt_flow_ts[gt_start_frame_index]
        total_dt += final_dt

        final_gt_dt = self.gt_flow_ts[gt_start_frame_index + 1] - self.gt_flow_ts[gt_start_frame_index]
    ...
    def get_single_item(self, ind):
        ...
        if self.train:
            if self.stack_mode == "SBT":  # 按照固定时间间隔取
                ...
                while n_events < self.min_n_events:
                    ...
                    frame_interval =self.SBT_time
                    n_iters += 1

                    flag = False
                    try_count = 0
                    target_area = self.image_size[0] * self.image_size[1]
                    full_area = 260 * 346
                    frame_start_event_index = ind
                    self.target_events_num = 0.6 * (target_area / full_area) * n_events
                    while flag == False:
                        bbox = self.get_box()  # 获取小图片的位置索引
                        # 这里不管什么模式，都是要取DVS的数据
                        events_flow = self.get_events(frame_start_event_index, frame_end_event_index,
                                                      bbox)  # 取出对应位置和跨越这几帧的事件
                        if try_count >= 6:
                        ...
                        if events_flow.shape[0] >= self.target_events_num:
                            flag = True
                    final_frame_interval=self.SBT_time
                    if n_events < self.min_n_events or flag == False:  # or ind%5==0:  # 如果取到的帧事件数量太小，则跳过，随机再取一个帧
                ...
                while frame_interval > self.max_skip_frames * self.SBT_time:
                    ...
                    if frame_interval > self.max_skip_frames * self.SBT_time or flag == False:  # 如果间隔太大，证明这段事件太稀疏
                        ind = random.randint(0, self.__len__())
                        frame_interval = 999 * self.SBT_time

                    # frame_end_event_index = ind + self.SBN_num // 2
                    # frame_start_event_index = ind - self.SBN_num // 2
                    ...
                    #     ind = random.randint(self.SBN_num // 2, self.__len__())
            ...
            if self.stack_mode == "SBT":  # 按照固定时间间隔取
                ...
                #         getdata_pose_index = random.randint(0, int(self.__len__()))
                #         frame_interval = 99999 * self.SBT_time
        events_flow = self.get_events(frame_start_event_index, frame_end_event_index, bbox)
        ...
        avg_frame_interval = (self.img_ts[-1]-self.img_ts[0])/self.img_ts.shape[0]
        time_scaling=final_frame_interval/(1 / 45.)


        # test=avg_frame_interval

        # elapsed = (time.time() - start_time)
        # print("取数据用时:", elapsed," ind=",ind)
