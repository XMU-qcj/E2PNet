...
def warp_images_with_flow(image, flow):
    ...
    pos_x = coord_x.unsqueeze(0).unsqueeze(0) + flow_y#经过验证，要反过来
    pos_y = coord_y.unsqueeze(0).unsqueeze(0) + flow_x
...
class DeepSupervise(nn.Module):
    ...
    def forward(self, image_frames, pred, gt_flow_frame, outer_mask=None, margin_mask=None):
        ...
        if outer_mask != None:  # 预测和GT先乘以mask后再计算loss
            ...
            losses_photometric = [self.w_photometric * warping_flow_loss(scalar_frames[i][0], scalar_frames[i][1]
                                                                         , pred[i])
                                  for i in range(len(pred))]#测试损失函数
