...
class H5Loader(BaseDataLoader):
    def __init__(self, config, num_bins):
        ...
        self.batch_idx = [i for i in range(self.config["batch"])]  # event sequence
        self.batch_row = [0 for i in range(self.config["batch"])]  # event_idx / time_idx / frame_idx
        self.batch_t0 = [None for i in range(self.config["batch"])]
    ...
    def __getitem__(self, index):
        while True:
            ...
            # trigger sequence change
            if (self.config["data"]["mode"] == "events" and xs.shape[0] < self.config["data"]["window"]) or xs.shape[0] <= 10:
                restart = True
