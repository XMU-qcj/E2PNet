...
data_file_list=["indoor_flying1_full_new.h5",
              "indoor_flying2_full_new.h5",
              "indoor_flying3_full_new.h5",
              "indoor_flying4_full_new.h5",
              "outdoor_day1_full_new.h5",
              "outdoor_day2_full_new.h5",]
...
class Data_MVSEC(Dataset):  # 这个类用于准备数据（取出所有数据）
    ...
    def load(self):
        if len(self.data_file_list)==1:
            self.hdf5_file_patch = os.path.join(self.folder_path, 'MVSEC')
            self.hdf5_file_patch = os.path.join(self.hdf5_file_patch, self.data_file_list[0])
            ...
            self.depth_image_rect = H5file['davis/left/depth_img']

            self.pose = H5file['davis/left/odometry']  # 与上面是两种等效的表达方式，上面的更高效
            self.pose_ts = H5file['davis']['left']['odometry_ts'][:]  # pose是全局对准的位姿
            ...
            for data_file in self.data_file_list:
                ...
                if flag < 1:
                    self.events = H5file['davis/left/events'] [:] # [w,h,t,p]
                    self.events_time = self.events[:, 2]
                    self.gt_flow_ts = H5file['davis']['left']['gtflow_ts'] [:]# gtflow是根据odo生成的
                    self.gt_flow_frame = H5file['davis']['left']['gtflow'][:]
                    self.depth_image_rect = H5file['davis/left/depth_img'][:]
                    self.pose = H5file['davis/left/gtflow'] [:] # 与上面是两种等效的表达方式，上面的更高效
                    self.pose_ts = H5file['davis']['left']['gtflow_ts'][:]  # pose是全局对准的位姿
                    self.image_frame = H5file['davis']['left']['img_frame'][:]
                    self.image_ts = H5file['davis']['left']['image_ts'][:]
                    self.image_event_index = H5file['davis']['left']['image_event_index'][:]
                    ...
                    self.events_time = self.events[:, 2]
                    self.gt_flow_ts = np.concatenate((self.gt_flow_ts,H5file['davis']['left']['gtflow_ts'][:] ),axis=0) # gtflow是根据odo生成的
                    self.gt_flow_frame = np.concatenate((self.gt_flow_frame,H5file['davis']['left']['gtflow'][:]),axis=0)
                    self.depth_image_rect = np.concatenate((self.depth_image_rect,H5file['davis/left/depth_img'][:]),axis=0)
                    self.pose = np.concatenate((self.pose,H5file['davis/left/gtflow'][:]),axis=0)  # 与上面是两种等效的表达方式，上面的更高效
                    self.pose_ts = np.concatenate((self.pose_ts,H5file['davis']['left']['gtflow_ts'][:] ),axis=0) # pose是全局对准的位姿
    ...
    def get_single_item(self, getdata_pose_index):#取出第ind笔(位姿)对应的数据
        ...
        # sample_index=np.random.choice(valid_points.shape[0], self.pcd_num // 8, replace=True)
        # valid_points=valid_points[sample_index]#点云随机采样为固定数量
        # sample_index = np.random.choice(enlarged_valid_points.shape[0], self.pcd_num // 8, replace=True)
