...
def create_update(x, y, t, dt, p, vol_size):
    ...
    vol_mul = torch.where(p < 0,
                          torch.div(torch.ones(p.shape, dtype=torch.long).to(x.device) * vol_size[0] , 2, rounding_mode='trunc'),
...
def create_batch_update(x, dx, y, dy, t, dt, p, vol_size):
    ...
    vol_mul = torch.where(p < 0,
                          torch.div(torch.ones(p.shape, dtype=torch.long).to(x.device) * vol_size[1], 2, rounding_mode='floor'),
                          torch.zeros(p.shape, dtype=torch.long).to(x.device))
